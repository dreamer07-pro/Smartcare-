...
function App(){
  ...
  async function auth(e){
    ...
    try{
      if(!supabase) throw new Error('Supabase is not configured. Add the frontend Supabase URL and anon key to frontend/.env.');
      ...
      if(mode==='signup'){
        ...
        if(data.user){
          const {error:pe}=await supabase.from('profiles').insert({id:data.user.id,name,email,role});
          if(pe) throw pe;
          if(role==='doctor'){
            const {error:de}=await supabase.from('doctors').insert({user_id:data.user.id,specialization:'General Medicine',available_slots:['09:00','09:30','10:00','10:30','11:00']});
            if(de) throw de;
          }
        }
        alert('Account created. If email confirmation is enabled, verify your email and then login.');
        setMode('login');
      } else {
  ...
  if(user) return user.role==='doctor'
    ? <Doctor user={user} logout={()=>{supabase?.auth.signOut();setUser(null)}}/>
    : <Patient user={user} logout={()=>{supabase?.auth.signOut();setUser(null)}}/>;
  ...
  return <div className="auth"><div className="auth-card">
    ...
    {!supabase && <div className="demo">Configure Supabase in frontend/.env before logging in.</div>}
  </div></div>
}
...
function Patient({user,logout}){
 ...
 const load=async()=>{try{
   ...
   setApps(await api(`/patient/${user.id}/appointments`));
   setReminders(await api(`/reminders/${user.id}`));
 ...
 return <Shell user={user} logout={logout} title="Patient Dashboard">
  ...
  <div className="grid">
   <section className="card"><h2>📅 Book Appointment</h2><form onSubmit={book}>
    ...
    <button className="primary" disabled={!doctors.length}>Book & Get Token</button>
   </form>{!doctors.length&&<p className="muted">No doctors found. Add doctors in Supabase first.</p>}</section>
   <section className="card highlight"><h2>🎟️ My Queue</h2>{apps.length?apps.slice(0,3).map(a=><div className="queue-box" key={a.id}><div className="token">Q-{a.token_number}</div><div><b>{a.status}</b><p>{a.date} · {a.time_slot}</p><span className="pill">{a.status==='waiting'?'Queue active':''}</span></div></div>):<p>No appointments yet.</p>}</section>
  </div>
...
function Doctor({user,logout}){
 const [queue,setQueue]=useState([]),[doctorId,setDoctorId]=useState(null),[notes,setNotes]=useState({});
 const today=new Date().toISOString().slice(0,10);
 useEffect(()=>{(async()=>{try{const ds=await api('/doctors');const me=ds.find(d=>d.user_id===user.id);if(!me){alert('Doctor profile not found.');return}setDoctorId(me.id);setQueue(await api(`/queue/${me.id}/${today}`))}catch(e){alert(e.message)}})()},[]);
